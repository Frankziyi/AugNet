...
from utils.Dataset import Dataset, DatasetAugThree, DatasetAug
...
image_datasets['train'] = DatasetAugThree(image_dir, data_transform, data_transform_resize, data_transform_resize2, data_transform_resize2)
...
def save_network2(network1, network2, epoch_label):
    ...
    save_filename3 = 'net3_%s.pth'% epoch_label
    save_path3 = os.path.join(args.model_save_dir, save_filename3)
    torch.save(network3.state_dict(), save_path3)



def train_model(model, optimizer, scheduler, optimizer2, scheduler2, optimizer3, scheduler3, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for data in dataloaders:
            inputs, inputs2, inputs3, inputs4,  _, _ = data
        ...
        epoch_loss3 = running_loss3 / len(dataloaders)
        epoch_l2_loss = l2_loss / len(dataloaders)
        epoch_l2_loss2 = l2_loss2 / len(dataloaders)
        #pdb.set_trace()
	#print ('Epoch:{:d} TripletLoss1: {:.4f}  TripletLoss2: {:.4f}  L2Loss: {:.4f}'.format(epoch, epoch_loss1, epoch_loss2, epoch_l2_loss))
        print ('Epoch:{:d} TripletLoss1: {:.4f}  TripletLoss2: {:.4f}  TripletLoss3: {:.4f}  L2Loss: {:.4f}  L2Loss2: {:.4f}'.format(epoch, epoch_loss1, epoch_loss2, epoch_loss3, epoch_l2_loss, epoch_l2_loss2))
        #save_network(model, 'test')

	if (epoch + 1) % 5 == 0:
            save_network2(model, model2, model3, epoch)

model = train_model(model, optimizer_ft, exp_lr_scheduler, optimizer_ft2, exp_lr_scheduler2, optimizer_ft3, exp_lr_scheduler3, args.num_epochs)
